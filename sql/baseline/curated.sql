...
-- BP3 restates CONFIG.BILLING_SCHEDULE's period rule rather than reading its output,
-- so the two can be compared independently. That means it must restate the rule
-- *exactly*, including the upfront case: cadence NULL or 0 is one period spanning the
-- term. The earlier form divided by NULLIF(cadence,0), which for an upfront contract
-- produced an expected value of NULL, and NULL never equals the actual 1, so this
-- check reported FAIL on a pipeline that was computing the right answer.
